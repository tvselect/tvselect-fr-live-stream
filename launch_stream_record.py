...
def get_tf1_credentials():
    """Retrieve TF1 credentials from keyring if CRYPTED_CREDENTIALS is enabled."""
    if not CRYPTED_CREDENTIALS:
        return False

    try:
        username = keyring.get_password("tf1", "username")
        password = keyring.get_password("tf1", "password")

        if username is None:
            logger.error("Failed to retrieve 'username' from keyring for 'tf1'.")
            return False

        if password is None:
            logger.error("Failed to retrieve 'password' from keyring for 'tf1'.")
            return False
...
def can_process_tf1_video(TF1_EMAIL, TF1_PASSWORD, channel):
    ...
    if CRYPTED_CREDENTIALS:
        ...
        if TF1_EMAIL == "XXXXXXXXXX" or TF1_PASSWORD == "XXXXXXXXXX":
            logger.error(
                f"The channel '{channel}' cannot "
                "be recorded because TF1 credentials are "
                "not set in config.py."
            )
            return False
        else:
            return True
...
for video in data:
    ...
    with open(log_file, "a", encoding="utf-8") as log:
        launch = subprocess.Popen(
            ...
            stdout=log,
            stderr=log,
            env=secure_env_with_creds if video["channel"] in TF1_CHANNELS else safe_env,
        )
